...
class ConcentrationRangeFeatures(BaseEstimator, TransformerMixin):
    ...
    def _learn_data_characteristics(self, X: pd.DataFrame, y: Optional[pd.Series] = None):
        ...
        logger.info(f"[DEBUG] Total columns: {len(X.columns)}")
        logger.info(f"[DEBUG] First 20 column names: {list(X.columns[:20])}")

        # Debug: Show all available K_I columns
        ...
        logger.info(f"[DEBUG] K_I intensity columns matching pattern: {k_intensity_cols[:5]}...")
        ...
        if 'B_C_ratio' in X.columns:
            ...
            self.mgc_ratio_percentiles_ = {
                ...
                'p50': np.percentile(mgc_ratios, 50),
                'p75': np.percentile(mgc_ratios, 75),
    ...
    def _add_ratio_adjustments(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'B_C_ratio' in X.columns and hasattr(self, 'mgc_ratio_percentiles_') and self.mgc_ratio_percentiles_:
            ...
            ratio_adjustment = 1.0 + 0.5 * np.abs(mgc_ratio - self.mgc_ratio_percentiles_['p50']) / (
                self.mgc_ratio_percentiles_['p75'] - self.mgc_ratio_percentiles_['p25'] + 1e-6)
            X['mgc_ratio_concentration_adjusted'] = mgc_ratio * ratio_adjustment

            # Extreme ratio indicator
            extreme_ratio_mask = (mgc_ratio <= self.mgc_ratio_percentiles_['p10']) | \
...
def create_enhanced_feature_pipeline_with_concentration(config, strategy: str, exclude_scaler: bool = False, use_parallel: bool = False, n_jobs: int = -1):
    ...
    logger.info(f"[DEBUG] config.use_raw_spectral_data = {config.use_raw_spectral_data}")
    logger.info(f"[DEBUG] strategy = {strategy}")
