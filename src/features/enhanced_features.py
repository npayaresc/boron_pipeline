...
Uses potassium (K) and other elements as context for improved magnesium prediction.
"""
import logging
...
class AdvancedRatioCalculator:
    """Calculates advanced elemental ratios for magnesium prediction with potassium as context."""
    ...
    def calculate_nutrient_ratios(self, features: Dict[str, float]) -> Dict[str, float]:
        """Calculate various nutrient ratios relevant to crop magnesium prediction."""
        ratios = {}

        # Define key features for ratio calculations
        key_features = ['K_I_simple_peak_height', 'K_I_peak_0', 'C_I_simple_peak_height', 'CA_I_422_simple_peak_height', 'N_I_help_simple_peak_height']
        ...
        # Get peak intensities using available feature format
        # Note: This is for MAGNESIUM prediction - K and other elements serve as context

        # Primary Mg lines (PREDICTION TARGET)
        mg_285_intensity = get_intensity(features, 'Mg_I_285')  # Mg I at 285.2 nm (strong neutral line)
        mg_383_intensity = get_intensity(features, 'Mg_I_383')  # Mg I at 383.8 nm
        mg_ii_intensity = get_intensity(features, 'Mg_II')  # Mg II ionic lines (279.55, 279.80, 280.27 nm)

        # K lines for CONTEXT (nutrient balance, not prediction target)
        ...
        # Additional K lines at 404 nm region (context)
        ...
        # Other context elements for nutrient balance
        s_intensity = get_intensity(features, 'S_I')
        ca_intensity = get_intensity(features, 'CA_I_422')  # Ca I at 422.673 nm (strongest Ca I line)
        ...
        # Critical ratios for MAGNESIUM prediction (using K and other elements as context)
        # Note: Mg_C_ratio is already created in base feature engineering, so we skip it here

        # K/Ca ratios (cation competition - context for Mg prediction)
        ratios['K_Ca_ratio'] = safe_ratio(k_primary_intensity, ca_intensity, 'K/Ca')  # K-Ca competition
        ...
        # K line intensity ratios (plasma diagnostics - context)
        ...
        # K vs nutrients (nutrient balance indicators - context for Mg prediction)
        ratios['K_N_ratio'] = safe_ratio(k_primary_intensity, n_intensity, 'K/N')  # Plant metabolism context
        ratios['K_P_ratio'] = safe_ratio(k_primary_intensity, p_intensity, 'K/P')  # Nutrient balance context
        ratios['K_S_ratio'] = safe_ratio(k_primary_intensity, s_intensity, 'K/S')  # Secondary nutrient balance

        # Combined cation ratios (context)
        ...
        # Mg ratios (PRIMARY - these directly relate to the prediction target)
        if not np.isnan(mg_285_intensity):
            ratios['K_Mg_ratio'] = safe_ratio(k_primary_intensity, mg_285_intensity, 'K/Mg')  # K-Mg competition
            ratios['Mg_K_ratio'] = safe_ratio(mg_285_intensity, k_primary_intensity, 'Mg/K')  # Inverse

        # Base saturation indicators (for Mg prediction)
        ...
        if total_bases > 0:
            ratios['K_base_fraction'] = safe_ratio(k_primary_intensity, total_bases, 'K/TotalBases')  # Context

        # Combined K intensity (total potassium signal - context)
        k_total = sum(x for x in [k_primary_intensity, k_secondary_intensity, k_404_intensity, k_404_2_intensity] if not np.isnan(x))
        if k_total > 0:
            ratios['K_total_C_ratio'] = safe_ratio(k_total, c_intensity, 'KTotal/C')  # Context
            ratios['K_total_intensity'] = k_total  # Total K signal strength (context)
...
class SpectralPatternAnalyzer:
    ...
    def calculate_peak_fwhm(self, wavelengths: np.ndarray, intensities: np.ndarray,
        ...
        """Calculate Full Width at Half Maximum for a spectral peak with adaptive windowing."""
        ...
        # Try progressively larger windows if needed to get sufficient points
        for attempt_window in [window, window * 1.5, window * 2.0, window * 3.0]:
            mask = (wavelengths >= peak_wavelength - attempt_window) & (wavelengths <= peak_wavelength + attempt_window)
            n_points = np.sum(mask)

            if np.any(mask) and n_points >= 5:
                break
        else:
            # Even with largest window, couldn't find enough points
            return {'fwhm': np.nan, 'asymmetry': np.nan}

        # If we still don't have enough points after all attempts, return NaN
...
class InterferenceCorrector:
    ...
    def detect_fe_interference(self, wavelengths: np.ndarray, intensities: np.ndarray,
        ...
        """Detect potential Fe interference on potassium context lines (used for Mg prediction)."""
        if fe_lines is None:
...
class EnhancedSpectralFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X_features: Dict[str, float], wavelengths: np.ndarray, 
        ...
        if self.config.enable_spectral_patterns:
            ...
            for element in ['Mg_I_285', 'Mg_II', 'CA_I_422', 'K_I', 'P_I_secondary']:
