...
class ObjectiveConfig(BaseModel):
    ...
    low_concentration_threshold: float = 0.15  # 15% Mg - low end
    medium_concentration_threshold: float = 0.35  # 35% Mg - high end
    high_concentration_threshold: float = 0.45  # 45% Mg - very high
...
class Config(BaseModel):
    ...
    macro_elements: List[PeakRegion] = [
        # Sulfur - S I 921-924 nm triplet is the strongest S region (NIST-verified for plant LIBS)
        PeakRegion(element="S_I", lower_wavelength=920.5, upper_wavelength=924.5, center_wavelengths=[921.287, 922.809, 923.752]),
        PeakRegion(element="S_I_2", lower_wavelength=868.0, upper_wavelength=870.0, center_wavelengths=[869.0]),
        # Phosphorus - P I 253.565 nm is THE standard P line in LIBS (NIST-verified for plant analysis)
        PeakRegion(element="P_I_secondary", lower_wavelength=251.0, upper_wavelength=256.0, center_wavelengths=[253.565]),  # WIDENED: 1.5→5.0 nm (was too narrow for FWHM calculation)

        # Note: Primary K and Mg lines are already defined in context_regions
        # to avoid duplication. Additional lines for macro analysis if needed:
