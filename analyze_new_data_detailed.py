#!/usr/bin/env python3
"""Detailed analysis of NEW data using pipeline configuration."""
...
# Import configuration from pipeline
...
# Extract element name from target column for display
element_name = config.target_column.split()[0]  # e.g., "Mg" from "Mg 285.213 (wt%)"

print("=" * 100)
print(f"{element_name.upper()} PIPELINE - DETAILED NEW DATA ANALYSIS")
print(f"Configuration from pipeline_config.py:")
print(f"  Raw data: {config.raw_data_dir_path}")
print(f"  Reference: {config.reference_data_path.name}")
...
# Analyze target element column
print("\n" + "-" * 100)
print(f"{element_name} Data Analysis:")
print("-" * 100)

target_col = config.target_column
if target_col in df_ref.columns:
    target_data = df_ref[target_col].dropna()
    print(f"\n{target_col}:")
    print(f"  Valid samples: {len(target_data)}/{len(df_ref)} ({len(target_data)/len(df_ref)*100:.1f}%)")
    print(f"  Missing samples: {df_ref[target_col].isna().sum()}")
    ...
    print(f"  Q1 / Q3: {target_data.quantile(0.25):.4f} / {target_data.quantile(0.75):.4f}")

    # Show rows with target values and their IDs
    print(f"\n  Sample rows with {element_name} values (first 10):")
    df_with_target = df_ref[df_ref[target_col].notna()][id_cols + [target_col]].head(10)
    print(df_with_target.to_string(index=False))
else:
    print(f"\n⚠️ WARNING: Target column '{target_col}' not found in reference file!")
...
# Filename pattern example: MPN_0000_002025_CNGS7330000001_POT_0000_S00531_P_Y_27062025_1000_0_01.csv.txt
# Note: "POT" is part of the instrument file naming convention, not element-specific
# Extract CNGS numbers and S numbers (sample identifiers)
sample_ids_extracted = []
sample_numbers = []
cngs_numbers = []

# Flexible pattern that works with various file naming conventions
pattern = r'(CNGS\d+).*?(S\d{5,})'

for f in all_files:
    filename = f.name

    # Try to extract CNGS number and S number using flexible pattern
    match = re.search(pattern, filename)
    if match:
        cngs = match.group(1)  # CNGS number (e.g., CNGS7330000001)
        sample_s = match.group(2)  # S number (e.g., S00531)
...
if target_col in df_ref.columns:
    print(f"  • Samples with {element_name} values: {df_ref[target_col].notna().sum()} ({df_ref[target_col].notna().sum()/len(df_ref)*100:.1f}%)")
    print(f"  • Missing {element_name} values: {df_ref[target_col].isna().sum()}")
    print(f"  • {element_name} concentration range: {target_data.min():.3f} - {target_data.max():.3f}%")
...
if target_col in df_ref.columns and len(target_data) > 0:
    print(f"\n📈 Statistical Summary:")
    ...
    print(f"  25%:      {target_data.quantile(0.25):.4f}%")
    print(f"  50%:      {target_data.median():.4f}%")
    print(f"  75%:      {target_data.quantile(0.75):.4f}%")
    print(f"  Max:      {target_data.max():.4f}%")

    print(f"\n📊 Concentration Ranges:")
    # Define ranges dynamically based on data range
    data_min, data_max = target_data.min(), target_data.max()
    data_range = data_max - data_min

    # Create adaptive ranges
    if data_range < 1.0:  # For narrow ranges (e.g., 0.2-0.5)
        ranges = [
            ("Very Low", data_min, data_min + data_range * 0.2),
            ("Low", data_min + data_range * 0.2, data_min + data_range * 0.4),
            ("Medium", data_min + data_range * 0.4, data_min + data_range * 0.6),
            ("High", data_min + data_range * 0.6, data_min + data_range * 0.8),
            ("Very High", data_min + data_range * 0.8, data_max),
        ]
    else:  # For wider ranges (e.g., 0-15%)
        ranges = [
            ("Zero", 0, 0),
            ("Very Low", 0, 0.1),
            ("Low", 0.1, 0.5),
    ...
    for label, min_val, max_val in ranges:
        if max_val == 0:
            count = (target_data == 0).sum()
        elif max_val == float('inf'):
            count = (target_data >= min_val).sum()
        else:
            count = ((target_data >= min_val) & (target_data < max_val)).sum()
        pct = count / len(target_data) * 100
        print(f"  {label:15s} ({min_val:5.2f} - {max_val:5.2f}%): {count:4d} samples ({pct:5.1f}%)")
    ...
    # Main title
    fig.suptitle(f'NEW Data - {element_name} Concentration Analysis\n' +
                 f'{ref_file.name} ({len(target_data)} samples)',
    ...
    ax1.axvline(target_data.mean(), color='red', linestyle='--', linewidth=2, label=f'Mean: {target_data.mean():.2f}%')
    ax1.axvline(target_data.median(), color='green', linestyle='--', linewidth=2, label=f'Median: {target_data.median():.2f}%')
    ax1.set_xlabel(f'{element_name} (%)', fontsize=12)
    ...
    ax2.set_ylabel(f'{element_name} (%)', fontsize=12)
    ax2.set_title('Box Plot', fontsize=13, fontweight='bold')
    ax2.grid(True, alpha=0.3, axis='y')

    # Add quartile labels
    q1, q2, q3 = target_data.quantile([0.25, 0.5, 0.75])
    ...
    ax3.set_xlabel(f'{element_name} (%)', fontsize=11)
    ...
    stats.probplot(target_data, dist="norm", plot=ax4)
    ax4.set_title('Q-Q Plot (Normal)', fontsize=12, fontweight='bold')
    ax4.grid(True, alpha=0.3)

    # 5. Violin plot
    ax5 = fig.add_subplot(gs[1, 2])
    parts = ax5.violinplot([target_data], vert=True, showmeans=True, showmedians=True, showextrema=True)
    ax5.set_ylabel(f'{element_name} (%)', fontsize=11)
    ...
    for i, (bar, count) in enumerate(zip(bars, range_counts)):
        if count > 0:
            ...
            ax6.text(bar.get_x() + bar.get_width()/2., height,
                    f'{count}\n({count/len(target_data)*100:.1f}%)',
                    ha='center', va='bottom', fontsize=10, fontweight='bold')

    plt.tight_layout()
    output_file = Path("reports/NEW_data_detailed_analysis.png")
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    print(f"✓ Saved: {output_file}")
    plt.close()
else:
    print(f"\n⚠️ WARNING: Cannot create visualizations - target column '{target_col}' not found or has no data!")
